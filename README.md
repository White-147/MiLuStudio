...
当前仓库采用前后端、Worker、Python Production Skills 和桌面端分离的工程结构。Stage 15 已把 Electron 桌面宿主作为独立交付壳接入：桌面端只负责承载 Web UI、启动本地 Control API / Worker、注入 Control API base URL、展示 preflight 和生成 Windows 安装包，不绑定数据库 schema、migration 或业务文件系统。Stage 16 已补齐本地 deterministic 账号注册、登录、会话和设备绑定；许可证、激活码和付费码相关体验已从当前网页端与桌面端撤下，作为后续商业化大后期内容保留边界。Stage 17 已完成生产控制台可编辑能力。Stage 18 已完成真实 provider adapter 前配置页。Stage 19 已完成桌面发布验收与代码签名前置准备。Stage 20 已把 Web 主入口重构为 Codex 式工作台：左侧历史项目、中央单输入框、右侧进度与结果预览卡、阶段门禁加号上传菜单，以及左下角统一设置入口。Stage 21 已完成新工作台结构化产物编辑增强。Stage 22 已完成 provider adapter 安全前置层。Stage 23A 已推进 OpenAI-compatible baseUrl + API key 配置、DPAPI 本地密钥、连接测试、项目内 FFmpeg runtime 安装脚本、真实上传资产保存、文本 / DOCX / 图片 / 视频技术解析和审核回退。Stage 23B 已正式确认为文档 / 媒体深度解析与上传策略加固阶段，并已完成稳定 metadata / 文本切片 / PDF 嵌入文本探测 / DOC 结构化降级 / 媒体派生策略、后端可恢复分片上传 session API，以及 asset analysis 解析详情消费接口。2026-05-15 已完成 Stage 23B-P0 到 Stage 23B-P9 的 SQLite 本地持久化、Web dev 编排、设置 / 依赖入口、工作台轻量化、composer / 侧栏 / 搜索 / 主题等补丁。Stage 23C 已开始：P0 已让前端上传消费 Control API upload-sessions 并在工作台消费 asset analysis 详情；P1 已补 Tesseract-compatible OCR runtime 离线导入/显式下载安装脚本、preflight tessdata 细节和正向 OCR 强制验证入口；P2 已让 Worker 的 `story_intake` 优先消费上传故事文本资产解析结果；P3 已补 Poppler `pdftoppm` PDF rasterizer runtime 探测、离线导入脚本、preflight 检测项，以及扫描 PDF 后端 rasterize -> OCR wrapper 消费路径；P4 已补 DOCX 后端结构化 ZIP/XML 解析，记录段落、表格、页眉页脚、脚注 / 尾注 / 批注摘要，并通过 asset analysis endpoint 暴露给工作台详情；P5 已补复杂 PDF embedded text 探测，能在后端 adapter 内解码简单 Flate 压缩 stream 后生成文本切片；P6 已让图片 / 视频 prompt builder 消费上传参考图 / 参考视频 asset analysis 摘要，不暴露本地路径、不读取媒体、不触发 provider。当前阶段不再把 `runtime\tesseract` 和 `runtime\poppler` 缺失作为功能推进限制，外部离线包后续补齐后再跑正向 runtime 验证。

> 说明：当前项目处于 MVP 工程搭建阶段。Stage 0 到 Stage 22 已完成，Stage 23A 已推进真实上传、基础解析、FFmpeg 缩略图 / 抽帧和审核回退，Stage 23B 已开始落地上传后处理地基：`assets.metadata_json` 记录 `stage23b_asset_analysis_v1`、文本 / DOCX / PDF 可生成 chunk manifest、DOCX 已在 Stage 23C-P4 升级为结构化 ZIP/XML 解析并记录 document structure、PDF 可做轻量嵌入文本探测、DOC 缺少 converter runtime 时返回结构化降级 metadata，图片 OCR 已接入后端 Tesseract-compatible CLI 调用路径，runtime 可用时会产出 OCR 文本和 chunk manifest，缺少 runtime 时返回 `runtime_not_configured`；扫描 PDF 已接入后端 Poppler `pdftoppm` rasterizer 调用路径，runtime 可用时可先输出页图再复用 OCR wrapper，缺少 Poppler 或 Tesseract 时返回结构化降级；图片 / 视频由后端 adapter 记录压缩、缩略图、抽帧和 review proxy 派生策略；后端已新增 `upload-sessions` 可恢复分片上传 API，chunk 临时文件只落在项目 `uploads\.upload-sessions` 内，complete 后复用现有 assets 登记与解析链路；后端也已新增 `GET /api/projects/{projectId}/assets/{assetId}/analysis`，让工作台详情或后续生产链路通过 Control API 消费 parser / OCR summary / content blocks / chunk manifest / document structure，而不是读取本地文件路径。Stage 23B-P2 已把浏览器 Web dev 入口补齐为可选 `npm run dev:local`，会先启动本地 Control API / Worker，再启动 Vite；单独启动 Vite 时也会显示明确的 Control API 不可达页面。当前已可通过 Electron + electron-builder + NSIS 生成 Windows 桌面安装包，在应用内通过 Control API 完成登录、注册、会话和设备绑定，并在 Codex 式工作台中上传解析文本 / 图片 / 视频附件、提交制作要求、跟踪生产进度、预览结果、编辑真实分镜产物，以及编辑角色 / 画风 / 图片提示词 / 视频提示词结构化产物。Web 已有 OpenAI-compatible provider 配置和连接测试；桌面发布验收脚本会识别当前安装器和主程序仍为 `NotSigned`；真实模型 provider 生成、扫描 PDF OCR 的 runtime 正向验证、真实图片 / 视频 / 音频生成、真实媒体质检和最终成片尚未接入。
...
- Stage 23B 后端可恢复分片上传 API 已落地，Stage 23C-P0 已让前端上传改走 create session -> PUT chunks -> complete，最终仍进入同一个 Control API 上传解析链路。
- Stage 23B asset analysis 消费接口已落地，Stage 23C-P0 已在工作台资产行接入详情面板：`GET /api/projects/{projectId}/assets/{assetId}/analysis` 只读取数据库中的 `metadata_json`，返回 parser、OCR summary、content blocks、chunk manifest、上传策略和 no-provider 边界，并避免把本地 `uploads/storage` 路径作为 UI 可消费契约。
- Stage 23C-P1 已新增 `scripts\windows\Install-MiLuStudioTesseract.ps1` 和 `scripts\windows\Test-MiLuStudioStage23COcrRuntime.ps1`，依赖中心会展示 `tesseractPath`、`tessdataPath`、语言包状态和安装脚本；当前无 runtime 时保持结构化降级，导入离线包后可用 `-RequireRuntime` 跑正向 OCR 验证。
- Stage 23C-P2 已让 production worker 的 `story_intake` 输入优先消费最新可用 `story_text` 资产的解析文本：上传 metadata 会记录 `productionInput.storyTextCandidate`，Worker 只在无可用资产候选时回退 `story_inputs.original_text`；新增 `scripts\windows\Test-MiLuStudioStage23CAssetConsumption.ps1` 做正向回归。
- Stage 23C-P3 已新增 Poppler `pdftoppm` PDF rasterizer 后端路径：`ControlPlaneOptions`、preflight、`Set-MiLuStudioEnv.ps1`、`Install-MiLuStudioPdfRasterizer.ps1` 和 `Test-MiLuStudioStage23CPdfRasterizer.ps1` 均已接入；PDF 无 embedded text 时会在后端尝试 rasterize 扫描页并复用 OCR wrapper，缺 runtime 时保持结构化降级。
- Stage 23C-P4 已把 DOCX 从简单抽取 `<w:t>` 升级为后端结构化 ZIP/XML 解析：段落、表格、页眉页脚、脚注 / 尾注 / 批注会写入 `technical.documentStructure`，analysis endpoint 和工作台详情 JSON 会同步暴露 `documentStructure`。
- Stage 23C-P5 已把 PDF embedded text probe 升级为 `stage23c_pdf_embedded_text_probe`：除明文 `Tj/TJ` 外，会尝试解码简单 `/Filter /FlateDecode` stream 并记录 `streamCount`、`decodedStreamCount` 和 `failedStreamCount`；缺 Poppler / OCR runtime 仍不阻塞该路径。
- Stage 23C-P6 已把上传参考图 / 参考视频 asset analysis 摘要接入生产链路：`image_prompt_builder` 和 `video_prompt_builder` 输入会携带 `asset_analysis`，Python skill 只消费 asset id、派生类型、探测摘要和 no-provider 边界，不读取本地媒体路径。
...
- Stage 23B 正式范围是文档 / 媒体深度解析与上传策略加固；已完成稳定 metadata schema、文本切片 manifest、PDF 嵌入文本探测、DOC 结构化降级、图片 OCR runtime 调用路径、图片 preview、视频抽帧 / review proxy、可恢复分片上传 endpoint、asset analysis 消费接口和验证脚本。Stage 23C-P0/P1/P2/P3/P4/P5/P6 已补前端分片上传消费、工作台资产详情消费、Tesseract runtime 导入/安装脚本、preflight tessdata 细节、OCR 正向验证 wrapper、production worker 对上传故事文本资产解析结果的实际消费、PDF rasterizer 后端调用路径、Poppler 离线导入脚本、DOCX 结构化 ZIP/XML 解析、PDF Flate stream embedded text 解码，以及参考图 / 视频 asset analysis 摘要进入图片 / 视频 prompt builder；当前不再让 Tesseract / Poppler 离线包缺失阻塞 Stage 23C 功能推进，正向 OCR / 扫描 PDF 验证待外部离线包补齐后执行。
...
- Stage 23B-P7 已完成搜索与设置收尾：项目搜索已从刷新按钮改为真实筛选；新项目入口保留当前空白草稿逻辑但补齐状态清理；模型配置和依赖配置继续通过 Control API 对接后端，同时清理开发态文案和过宽弹窗布局。
...
Stage 23B 已正式确认并开始落地。Stage 23B-P0 SQLite 本地持久化与开发稳定化补丁已完成；Stage 23B-P1 已完成检查与切分判断；Stage 23B-P2 已完成稳定化收尾；Stage 23B-P3 已完成设置入口和依赖入口补丁；Stage 23B-P4 已完成工作台 UI 视觉轻量化补丁；Stage 23B-P5 已完成品牌栏、composer 输入区和真实上传大小提示收尾；Stage 23B-P6 已完成进度卡去重和侧栏拖拽 / 收缩补丁；Stage 23B-P7 已完成项目搜索真实可用、新项目状态清理补齐、模型 / 依赖配置文案和弹窗可用性；Stage 23B-P8 已完成左侧项目行搜索交互、按钮对齐、菜单外部点击取消和设置按钮残留 UI 收口；P8 follow-up 已完成项目区文本左对齐、品牌栏与收缩按钮纵向对齐、搜索态外部点击退出、设置入口文字 / 图标减重和侧栏收缩 / 展开动效；Stage 23B-P9 已完成深浅色主题切换、浅色 token、左下主题切换入口和 XiaoLouAI 方向轻度配色对齐。Stage 23C-P0/P1/P2/P3/P4/P5/P6 已推进正式功能阶段第一批任务：前端分片上传消费、工作台资产详情消费、Tesseract runtime 导入/安装脚本、preflight 细节、OCR 验证 wrapper、生产链路实际消费上传故事文本资产解析结果、Poppler PDF rasterizer 后端路径、DOCX 结构化解析、PDF Flate stream 解析，以及参考图 / 视频 asset analysis 摘要进入 prompt builder。下一步继续：

1. Stage 23C：继续复杂 PDF 解析、扫描 PDF 正向 fixture 预留和媒体派生策略回归；Tesseract / Poppler 离线包后续补齐后，再分别跑 `Test-MiLuStudioStage23COcrRuntime.ps1 -RequireRuntime` 与 `Test-MiLuStudioStage23CPdfRasterizer.ps1 -RequireRuntime`。
