...
- Chain `story_intake` -> `plot_adaptation` -> `episode_writer` -> `character_bible` -> `style_bible` -> `storyboard_director` -> `image_prompt_builder` -> `image_generation` -> `video_prompt_builder` -> `video_generation` through runtime envelopes.
- Produce reviewable script, character, style, storyboard, image prompt, mock placeholder image asset, video prompt, and mock placeholder video clip structures without database or file writes.
